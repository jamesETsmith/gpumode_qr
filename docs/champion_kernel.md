...
winning kernel is built the way it is. We will build the idea in the order a
numerical linear algebra course would: first the problem and how we are graded,
then the classical tool (Householder reflectors), then the one performance idea
that makes everything else fall into place (blocking with the compact-WY
representation), and finally the specific kernel and the numbers it produces.
