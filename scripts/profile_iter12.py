...
from qrbench import inputs, EPS32  # noqa: E402
from qrbench.variants import (  # noqa: E402
    _batched_cholesky_fused, _batched_cholesky,
    _trsm_right_upper_fused, _trsm, _modified_lu_fused,
)
...
def profile(batch, n, cond, shift_coef=1.5, passes=2):
    ...
    def solveq(R, B):
        return _trsm_right_upper_fused(R, B, block=kb) if fused_trsm else _trsm(R, B, upper=True, left=False)
    ...
    total = (t_gemm + t_chol1 + t_solve1 + t_gemm_ref + t_chol_ref
             + t_solve_ref + t_guard + t_modlu + t_rstored + t_repair)
    ...
    print(f"  [chol total {chol_all:.2f}] [solve total {solve_all:.2f}] "
